...
def run(data,
    ...
    if training:  # called by train.py
        ...
        if cropped:
            ...
            lpd_stride, lpd_pt = lpd_model.stride, lpd_model.pt
            lpd_imgsz = check_img_size(lpd_imgsz, s=lpd_stride)  # check image size
            lpd_half = lpd_model.fp16  # FP16 supported on limited backends with CUDA
    ...
    if not training:
        ...
        if cropped:
            lpd_model.warmup(imgsz=(1 if lpd_pt else batch_size, 3, lpd_imgsz, lpd_imgsz))  # warmup
        pad = 0.0 if task in ('speed', 'benchmark') else 0.5
        lpd_pad = 0.0 if task in ('speed', 'benchmark') else 0.5
    ...
    if cropped:
        lpd_names = {k: v for k, v in enumerate(lpd_model.names if hasattr(lpd_model, 'names') else lpd_model.module.names)}
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        if opt.cropped:
            if opt.lpd_conf_thres > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
                LOGGER.info(emojis(f'WARNING: confidence threshold {opt.lpd_conf_thres} > 0.001 produces invalid results ⚠️'))
        else:
